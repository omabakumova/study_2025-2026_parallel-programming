...
program test_reduction
  ...
  integer(int32), parameter :: max_threads_num = 2
  ...
  length = 10**8
  ! Эта проверка требует как минимум один аргумент, иначе программа завершится с ошибкой.
  if (command_argument_count() <= 0) then
    error stop "Передайте программе название функции,&
    & которую хотите тестировать (sum, min, max)!"
  end if

  call get_command_argument(1, argv) ! Здесь программа читает первый аргумент (1) и сохраняет его в переменную argv.
